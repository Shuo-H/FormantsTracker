...
class Solver:
    ...
    def test(self):
        with torch.no_grad():
            for batch in tqdm(self.test_loader): # this loop only executed once ?
                spects,lengths,fnames = batch
                spects, lengths = spects.to(self.hp.device), lengths.to(self.hp.device)
                # call model.py FormantTracker
                out,_ = self.model(spects) # num of test wav files x num of formants x num of frames x num of frequencies ex: [2, 3, 479, 257]
                predictions=self.get_predicted_formants(out) # num of test wav files x num of formants x num of frames ex: [2, 3, 479]
    ...
    def get_predicted_formants(self,out):
        ...
        res = ((torch.max(out_smoothed, dim=3)[1])*self.bin_resolution)+self.bin_resolution/2
